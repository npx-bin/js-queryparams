    ...
    typeof window !== "undefined" &&
        (window[refName] = (function () {
            ...
            QueryParams.prototype.get = function (paramName) {
                if (paramsMap && paramsMap[paramName]) {
                    if (paramsMap[paramName].length === 1) {
                        return JSON.parse(
                            decodeURIComponent(JSON.stringify(paramsMap[paramName][0]))
                        );
                    } else {
                        return JSON.parse(
                            decodeURIComponent(JSON.stringify(paramsMap[paramName]))
                        );
                    }
                }
                return null;
            ...
            QueryParams.prototype.getAll = function () {
                return JSON.parse(
                    decodeURIComponent(JSON.stringify(paramsMap || null))
                );
            };
